...
<div class="space-y-6">
  ...
  <div class="group" id="emailGroup">
    ...
    <div class="relative">
      <input id="authEmail" class="w-full bg-surface-container-highest border-none rounded-md px-4 py-4 text-on-surface placeholder:text-on-surface-variant/30 focus:ring-2 focus:ring-primary-container transition-all outline-none" placeholder="editor@nqr.io" type="email" autocomplete="email" required/>
    </div>
  </div>
...
async function handleAuthSubmit(e, form) {
  ...
  if (currentStep === 'EMAIL') {
    ...
    if (!isValid) {
      errorMsg.textContent = 'Access restricted to @neverno.in domains only.';
      errorEl.classList.add('visible');
      return;
    }
    ...
    try {
      ...
      const res = await window.API.requestOtp(payload.email, payload.password);
